...
def data_generator(data_path, data_type, hparams, test_subset=True):
    # Load datasets
    ...
    # If test_subset is True, select 50% of the test data
    if test_subset:
        test_size = len(test_loader.dataset)
        indices = list(range(test_size))
        random.shuffle(indices)
        subset_indices = indices[:int(test_size * 1)]  # Select 50% of the data
        test_loader = torch.utils.data.DataLoader(Subset(test_loader.dataset, subset_indices), batch_size=batch_size, shuffle=False, num_workers=0)

    return test_loader, val_loader, test_loader, get_class_weight(cw_dict)
